...
export default function FilterPage() {
  ...
  useEffect(() => {
    axios.get(`${process.env.NEXT_PUBLIC_API_BASE_URL}vehicles/GetMakesForVehicleType/car?format=json`)
  ...
  const handleVehicleTypeChange = (e) => {
    ...
    const selectedType = vehicleTypes.find(type => String(type.MakeId) === selectedMakeId);
  ...
  return (
    <div
      className="flex flex-col items-center justify-center min-h-screen py-2 bg-cover bg-center bg-[url('/images/backgroundCar.png')]"
    >
  <h1 className="text-5xl font-extrabold mb-8 text-transparent text-center drop-shadow-lg text-stroke">
    CAR DEALER APP
      ...
      <div className="p-8 rounded-md shadow-lg bg-white bg-opacity-90">
        <div className="mb-4">
          <label className="block text-lg text-gray-800">Select Vehicle Type:</label>
          <select
        ...
        <div className="mb-4">
          <label className="block text-lg text-gray-800">Select Model Year:</label>
          <select
            className="border rounded-md p-2 w-full bg-white bg-opacity-80 text-gray-800"
            value={selectedYear}
        ...
        >
          <button
            className={`bg-blue-500 text-white p-2 rounded w-full ${
              !selectedVehicleType || !selectedYear ? 'opacity-50 cursor-not-allowed' : ''
            }`}
